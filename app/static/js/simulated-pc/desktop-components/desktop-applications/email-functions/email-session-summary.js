export class EmailSessionSummary {
    ...
    showSessionSummary(sessionStats, feedbackHistory = []) {
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-lg p-8 max-w-4xl mx-4 max-h-96 overflow-y-auto">
                <div class="text-center mb-8">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="text-lg text-gray-600">Email Security Accuracy</div>
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center mt-8">
                    ...
                    ` : `
                        <button onclick="window.emailSessionSummary?.retryTraining?.()" 
                                class="bg-orange-600 text-white px-8 py-3 rounded-lg hover:bg-orange-700 transition-colors font-semibold text-lg">
                ...
                <div class="text-center mt-6 pt-6 border-t border-gray-200">
                    <div class="text-sm text-gray-600 mb-3">Want to keep track of your progress?</div>
        ...
        if (accuracy >= 90) {
            return `
                <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                    <div class="text-green-800 font-semibold">Outstanding Performance!</div>
                    <div class="text-green-700 text-sm">You demonstrated exceptional email security awareness. You're ready for advanced challenges!</div>
                </div>
            `;
        } else if (accuracy >= 80) {
            return `
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    <div class="text-blue-800 font-semibold">Excellent Work!</div>
                    <div class="text-blue-700 text-sm">Strong email security skills. You've successfully completed this level!</div>
                </div>
            `;
        } else if (accuracy >= 70) {
            return `
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                    <div class="text-yellow-800 font-semibold">Good Progress!</div>
                    <div class="text-yellow-700 text-sm">You've passed the minimum threshold. Review the feedback to strengthen your skills.</div>
                </div>
            `;
        } else {
            return `
                <div class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                    <div class="text-red-800 font-semibold">Keep Learning!</div>
                    <div class="text-red-700 text-sm">Email security requires more practice. Review the feedback and try again to improve your skills.</div>
                </div>
            `;
        }
    }
        ...
        return `
            <div class="space-y-4">
                <div class="flex justify-between text-sm">
                    <span class="text-green-600 font-medium">Correct Actions</span>
                    <span class="font-semibold">${sessionStats.correctActions}</span>
                </div>
                ...
                <div class="flex justify-between text-sm">
                    <span class="text-red-600 font-medium">Incorrect Actions</span>
                    <span class="font-semibold">${sessionStats.totalActions - sessionStats.correctActions}</span>
                </div>
                <div class="w-full bg-gray-200 rounded-full h-3">
    ...
    generateEmailCategoriesSection(feedbackHistory) {
        ...
        return `
            <div class="bg-gray-50 rounded-lg p-6 mb-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                    <i class="bi bi-envelope-check text-purple-600 mr-2"></i>
                    ...
                    ${Object.entries(categories).map(([category, data]) => `
                        <div class="bg-white rounded-lg p-4 border border-gray-200">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="text-sm font-semibold ${data.accuracy >= 70 ? 'text-green-600' : 'text-red-600'}">
                                    ${data.accuracy}%
                                </span>
                            </div>
                ...
                <div class="space-y-3">
                    ${suggestions.map(suggestion => `
                        <div class="flex items-start">
                            ...
                            <div class="text-blue-800 text-sm">${suggestion}</div>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    reviewMistakes() {
        ...
        modal.innerHTML = `
            <div class="bg-white rounded-lg p-6 max-w-2xl mx-4 max-h-96 overflow-y-auto">
                <h2 class="text-xl font-bold text-gray-900 mb-4">Review Your Mistakes</h2>
                
                ${mistakes.length === 0 ? `
                    <div class="text-center py-8">
                        <div class="text-4xl mb-4">🎯</div>
                        <div class="text-lg font-semibold text-green-600">Perfect Score!</div>
                        <div class="text-gray-600">You didn't make any mistakes in this session.</div>
                    </div>
                ` : `
                    <div class="space-y-4">
                        ${mistakes.map((mistake, index) => `
                            <div class="border border-gray-200 rounded-lg p-4">
                                <div class="flex items-start justify-between mb-2">
                                    <div class="font-semibold text-gray-900">Email ${index + 1}</div>
                                    <div class="text-xs text-gray-500">${mistake.timestamp}</div>
                                </div>
                                ...
                                <div class="bg-red-50 border border-red-200 rounded p-3 mb-2">
                                    <div class="text-red-800 font-medium">Your Action: ${mistake.playerAction}</div>
                                    <div class="text-red-600 text-sm">This email was ${mistake.isSuspicious ? 'suspicious' : 'legitimate'}</div>
                                </div>
